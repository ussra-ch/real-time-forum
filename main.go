...
func main() {
	...
	http.HandleFunc("/api/post", handlers.RatelimitMiddleware(handlers.PostHandler, "posts", 2))
	http.HandleFunc("/api/fetch_posts", handlers.FetchPostsHandler)
	http.HandleFunc("/comment", handlers.RatelimitMiddleware(handlers.CommentHandler, "comments", 2))
